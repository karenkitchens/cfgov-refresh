...
import { show, hide, slide } from './repay-utils';
...
const quizHandler = function( container ) {
	...
	const handleButtons = () => {
		handler.questions.forEach( ( question ) => {
			const buttons = question.querySelectorAll( '.quiz__answer-choice' );
			buttons.forEach( ( buttonArea ) => {
...
const init = function() {
	...
	quizContainers.forEach( ( container ) => {
		let quiz = quizHandler( container );
	} );
